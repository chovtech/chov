  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      <input
        ...
        className="w-full px-3 py-2.5 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#00AE7E]/20 focus:border-[#00AE7E] transition-all"
      />
        ...
        <div className="absolute z-50 w-full mt-1 bg-white border border-slate-200 rounded-lg shadow-lg max-h-52 overflow-y-auto">
          {results.map(city => (
            <button
              ...
              className={`w-full text-left px-3 py-2 text-sm hover:bg-slate-50 transition-colors ${city === value ? 'font-semibold text-[#00AE7E]' : 'text-slate-700'}`}
            >
              {city}
            </button>
          ))}
