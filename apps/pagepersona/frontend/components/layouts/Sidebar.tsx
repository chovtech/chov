  ...
  return (
    ...
    <aside className="w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col fixed inset-y-0 left-0 z-50">
      ...
      {!isClientUser && <div className="relative px-3 pb-3">
        ...
        {workspaceOpen && (
          <div className="absolute top-full left-3 right-3 z-50 mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl overflow-hidden max-h-80 overflow-y-auto">
            {(() => {
              const myWorkspaces = workspaces.filter(ws => ws.parent_workspace_id === null)
              const clientWorkspaces = workspaces.filter(ws => ws.parent_workspace_id !== null)
              return (
                <div className="p-2">
